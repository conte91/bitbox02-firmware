...
__extension__ static uint16_t scroller_previous_position[][DEF_SCROLLER_NUM_PREV_POS] = {[0 ... (DEF_NUM_SCROLLERS - 1)][0 ... (DEF_SCROLLER_NUM_PREV_POS - 1)] = 0};
...
void qtouch_process_scroller_positions(void)
{
    ...
    for (scroller = 0; scroller < DEF_NUM_SCROLLERS; scroller++)
    {
        ...
        scaled_value = (scaled_value < DEF_SCROLLER_DEADBAND) ? DEF_SCROLLER_DEADBAND : (weighted_sum / sum);
        scaled_value = (scaled_value > (DEF_SCROLLER_RESOLUTION - DEF_SCROLLER_DEADBAND)) ? (DEF_SCROLLER_RESOLUTION - DEF_SCROLLER_DEADBAND) : scaled_value;
