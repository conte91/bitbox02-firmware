...
struct usb_processing {
    ...
    void (*manage_invalid_cmd)(struct queue* queue, uint32_t cid);
};
...
static void _usb_process_incoming_packet(struct usb_processing* ctx)
{
    ...
    if (!cmd_valid) {
        ctx->manage_invalid_cmd(ctx->out_queue(), _in_packet.cid);
    }
...
void usb_processing_init(void)
{
    ...
    usb_processing_u2f()->manage_invalid_cmd = u2f_invalid_api_command;
#endif
    ...
    usb_processing_hww()->manage_invalid_cmd = usart_invalid_api_command;
#else
    queue_init(queue_hww_queue(), USB_REPORT_SIZE);
    usb_processing_hww()->format_frame = usb_frame_reply;
    usb_processing_hww()->manage_invalid_cmd = usb_invalid_api_command;
#endif
