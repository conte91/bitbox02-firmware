...
class U2FUsart(TransportLayer):
    ...
    def write(self, data: bytes, dst_endpoint: int, cid: int) -> None:
        ...
        # print("Host -> Base (unpacked): endpoint {:02X}, data (len {}): ".format(dst_endpoint, len(data)) + data.hex())
    ...
    def read(self, endpoint: int, cid: int) -> bytes:
        ...
        data = data[2:]
        # print("Base -> Host (unpacked): cmd {:02X}, endpoint {:02X}, data (len {}): ".format(reply_cmd, src_endpoint, len(data)) + data.hex(), file=dump_file)
        return data
