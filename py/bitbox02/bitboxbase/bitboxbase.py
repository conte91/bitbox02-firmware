...
class BitBoxBase:
    ...
    def __init__(
        ...
        self.device = serial.Serial(device_info["path"], 115200, timeout=1)
    ...
    def _query(self, msg: bytes) -> bytes:
        ...
        """
        HWW_ENDPOINT = 1
        usart.write(self.device, msg, HWW_CMD, HWW_ENDPOINT, self.dump_file)
        return bytes(usart.read(self.device, HWW_CMD, HWW_ENDPOINT, self.dump_file))
    ...
    def insert_sdcard(self) -> None:
        ...
        request.insert_remove_sdcard.CopyFrom(
            bitbox02_system.InsertRemoveSDCardRequest(action=bitbox02_system.InsertRemoveSDCardRequest.INSERT_CARD)
        )
        self._msg_query(request, expected_response="success")
    ...
    def remove_sdcard(self) -> None:
        ...
        request = hww.Request()
        request.insert_remove_sdcard.CopyFrom(
            bitbox02_system.InsertRemoveSDCardRequest(action=bitbox02_system.InsertRemoveSDCardRequest.REMOVE_CARD)
        )
        self._msg_query(request, expected_response="success")
