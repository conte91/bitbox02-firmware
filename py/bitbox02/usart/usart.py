...
def write(usart_device: SupportsReadWrite, data: bytes, cmd: int, dst_endpoint: int) -> None:
    """
    ...
    data = bytes([1, dst_endpoint, cmd]) + bytearray(data)
    ...
    data_len = len(data)
    if data_len > 4096:
        raise ValueError("Data is too large 'size <= 4096'")

    to_write = encodeUsartFrame(data)
...
def read(usart_device: SupportsReadWrite, cmd: int, srcEndpoint: int) -> bytes:
    """
    ...
    buf = readUsartFrame(usart_device)
    ...
    data = data[3:]
    print("Response payload ({} bytes): {}".format(len(data), bytes2hex(data)))
    return data
