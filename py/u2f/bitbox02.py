...
class BitBox02U2F:
    ...
    def __init__(self, device: communication.TransportLayer):
        self._device = device
        # 0 and 0xffffffff are reserved
        self._cid = self._device.generate_cid()
        self.debug = False
    ...
    def _query(self, cid: int, cmd: int, msg: bytes) -> bytes:
        ...
        self._device.write(msg, cmd, cid)
        response_bytes = self._device.read(cmd, cid)
        if self.debug:
